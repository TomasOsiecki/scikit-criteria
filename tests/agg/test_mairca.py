...
def test_MAIRCA_with_custom_P_ai():
    ...
    # Custom P_ai values (not equal for all alternatives)
    P_ai = np.array([0.15, 0.10, 0.20, 0.15, 0.10, 0.10, 0.10, 0.10])
    
    # Expected results (calculated manually or from a reference implementation)
    Q_expected = np.array([0.0641, 0.0548, 0.1470, 0.0888, 0.0631, 0.0637, 0.0483, 0.0658])
    ranking_expected = np.array([2, 3, 8, 5, 6, 7, 1, 4])
    alternatives_expected = [f"A{i}" for i in range(len(matrix))]
    ...
    mairca = MAIRCA()
    result = mairca.evaluate(dm, P_ai=P_ai) #! FIX
...
def test_MAIRCA_zero_P_ai():
    ...
    # Verify the result is still valid (e.g., no division by zero)
